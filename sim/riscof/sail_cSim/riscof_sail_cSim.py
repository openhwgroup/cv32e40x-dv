...
class sail_cSim(pluginTemplate):
    ...
    def initialise(self, suite, work_dir, archtest_env):
        ...
        self.objdump_cmd = '{0}objdump -D {1} > {2};'
        self.archtest_env = archtest_env
        compile_cmd = '{0}gcc -march={1} \
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        self.gcc_prefix = "riscv{0}-{1}-elf-".format(self.xlen, self.sw_toolchain_prefix)
        ...
        objdump = "{0}objdump".format(self.gcc_prefix)

        if not self.docker:
            if shutil.which(objdump) is None:
                logger.error(objdump+": executable not found. Please check environment setup.")
                raise SystemExit(1)
            compiler = "{0}gcc".format(self.gcc_prefix)
    ...
    def runTests(self, testList, cgf_file=None):
        ...
        for file in testList:
            ...
            cmd = self.compile_cmd.format(self.gcc_prefix, self.isa) + ' ' + test + ' -o ' + elf
            compile_cmd = cmd + ' -D' + " -D".join(testentry['macros'])
            execute+=compile_cmd+";"

            execute += self.objdump_cmd.format(self.gcc_prefix, elf, 'ref.disass')
