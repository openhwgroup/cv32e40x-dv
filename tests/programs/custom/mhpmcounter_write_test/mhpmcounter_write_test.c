...
int check_mhpmcounters_are_ffff_ffff()
{
    uint32_t reg = 0;
    __asm__ volatile("csrr %0, mhpmcounter3" : "=r"(reg));
    if(reg != 0xffffffff) return 1;
...
int check_mhpmcounterhs_are_ffff_ffff()
{
    uint32_t reg = 0;
...
int main()
{
    ...
    is_failure += check_mhpmcounters_are_ffff_ffff();
    is_failure += check_mhpmcounterhs_are_ffff_ffff();
